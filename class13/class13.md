...
Today we will perform an RNSeq analysis on the effects of dexamethasone
...
compare the control vs treated to see which gene expression levels
change when we have the drug present.

We will go gene by gene and get an average for each control and
treatment group and do a comparison between these means.
...
> Q. Let’s do the same process for the treatment samples - find the mean
...
lead to “funky” log2 fold chnag values (e.g. divide by zero etc.)
...
The main function in this package is called `DESeq()` and it will run
the full analysis for us on our `dds` input object:
...
It is basically a plot of log2 fold change values vs the adjusted
p-values.

> Q. Use ggplot to make a first version volcano plot of `log2FoldChange`
> vs `padj`
...
Let’s start by mapping our ENSEMBLW ids to the more convential gene
SYMBOL.
...
> Q. Can you add “GENENAME” and “ENTREZID” as new columns to the `res`
> as “name” and “enterz”?
...
wants to main inputs: a vector of importance (in our case the log2fold
...
```
    ...
    Info: Working in directory /Users/nathanjoseph/Desktop/BIMM143/bimm143_github/class13
